...
const INTENT_LABELS_ES: Readonly<Record<IntentType, string>> = {
  create_reminder: "crear recordatorio",
  create_list: "crear lista",
  add_to_list: "añadir a lista",
  complete_list_item: "completar elemento de lista",
  remove_list_item: "eliminar elemento de lista",
  create_task: "crear tarea",
  complete_task: "completar tarea",
  create_memory_bubble: "guardar nota",
  query_park: "buscar en The_Park",
  query_office: "consultar The_Office",
  store_file: "guardar archivo",
  retrieve_file: "recuperar archivo",
  calendar_query: "consultar calendario",
  calendar_create: "crear evento de calendario",
  cancel_reminder: "cancelar recordatorio",
  list_reminders: "listar recordatorios",
  pause_reminder: "pausar recordatorio",
  resume_reminder: "reanudar recordatorio",
  update_reminder: "actualizar recordatorio",
  add_pre_notifications: "añadir pre-avisos",
  help: "mostrar ayuda",
  unknown: "intención desconocida",
};

function summarizeEntities(envelope: IntentEnvelope): string {
  const e = envelope.entities;
  const parts: string[] = [];
  if (e.content) parts.push(`«${e.content}»`);
  if (e.list_name) parts.push(`lista «${e.list_name}»`);
  if (e.recurrence_rule) parts.push(`con regla ${e.recurrence_rule}`);
  if (e.trigger_at) parts.push(`a las ${e.trigger_at}`);
  if (e.deadline_at) parts.push(`con plazo ${e.deadline_at}`);
  if (e.template_id) parts.push(`plantilla ${e.template_id}`);
  return parts.join(" ");
}

function formatConfirmation(envelope: IntentEnvelope): string {
  const intentLabel = INTENT_LABELS_ES[envelope.intent] ?? envelope.intent;
  const kindSuffix = envelope.reminder_kind
    ? ` (${envelope.reminder_kind})`
    : "";
  const summary = summarizeEntities(envelope);
  const head = `Entendí: ${intentLabel}${kindSuffix}`;
  const body = summary ? ` ${summary}` : "";
  return `${head}${body}. ¿Confirmas? Responde sí/no.`;
}
...
async function handleIntent(
  ...
  safeLog("info", "intent_dispatch", {
    intent: envelope.intent,
    reminder_kind: envelope.reminder_kind,
  ...
  switch (envelope.intent) {
    case "create_reminder":
      reply = await handleCreateReminder(envelope, owner.timezone);
      break;
    case "list_reminders":
      reply = await handleListReminders();
      break;
    case "cancel_reminder":
      reply = await handleCancelReminder(envelope);
      break;
    case "pause_reminder":
      reply = await handlePauseReminder(envelope);
      break;
    case "resume_reminder":
      reply = await handleResumeReminder(envelope, owner.timezone);
      break;
    case "update_reminder":
      reply = await handleUpdateReminder(envelope, owner.timezone);
      break;
    case "add_pre_notifications":
      reply = await handleAddPreNotifications(envelope, owner.timezone);
      break;
    case "create_list":
      reply = await handleCreateList(envelope);
      break;
    case "add_to_list":
      reply = await handleAddToList(envelope);
      break;
    case "complete_list_item":
      reply = await handleCompleteListItem(envelope);
      break;
    case "remove_list_item":
      reply = await handleRemoveListItem(envelope);
      break;
    case "query_office":
      reply = await handleQueryOffice(envelope);
      break;
    case "create_task":
      reply = await handleCreateTask(envelope);
      break;
    case "complete_task":
      reply = await handleCompleteTask(envelope);
      break;
    case "create_memory_bubble":
      reply = await handleCreateMemoryBubble(envelope);
      break;
    case "query_park":
      reply = await handleQueryPark(envelope);
      break;
    case "store_file":
      reply = "Para guardar un archivo, envíalo directamente como adjunto.";
      break;
    case "retrieve_file":
      reply = await handleRetrieveFile(envelope, chatId);
      break;
    case "calendar_query":
      reply = await handleCalendarQuery(envelope, owner.timezone);
      break;
    case "calendar_create":
      reply = await handleCalendarCreate(envelope, owner.timezone);
      break;
    case "help":
      await runCommand("/help", chatId, owner);
      return null;
    case "unknown":
      reply = "No entendí tu mensaje. ¿Puedes reformularlo?";
      break;
    default: {
      const exhaustive: never = envelope.intent;
      throw new Error(`unhandled intent: ${String(exhaustive)}`);
    }
  }

  const finalReply = envelope.entities.reply_text?.trim() || reply;
  try {
    await sendMessage(chatId, finalReply, { parse_mode: "Markdown" });
  } catch {
    await sendMessage(chatId, finalReply.replace(/[_*`\[\]]/g, ""));
  }

  if (
    envelope.entities.context_updates &&
    envelope.entities.context_updates.length > 0
  ) {
    for (const update of envelope.entities.context_updates) {
      if (update.category && update.key && update.value) {
        try {
          await db.from("owner_context").upsert({
            category: update.category,
            key: update.key,
            value: update.value,
            source: `conversación del ${
    ...
    safeLog("info", "context_updated", {
      keys: envelope.entities.context_updates.map((u) => u.key),
    });
  }
  return finalReply;
}
